...
void FileExplorer::on_openfile_dialog_response(int response_id, Gtk::FileChooserDialog* dialog)
{
    ...
    switch (response_id)
    {
        case Gtk::ResponseType::OK:
        {
            ...
            std::ifstream MyReadFile(current_filepath);
            std::string line, all;
            ...
            Application::win->m_refTextBuffer->set_text(all);
...
void FileExplorer::on_savefile()
{
    if(current_filepath.empty()) {
        //TODO Pop Up saying that there is no file opened
        std::cout << "[DEBUG]   There is no file opened" << std::endl;
        return;
    }
    ...
    std::ofstream MyFile(current_filepath);
